...
/*! Карта высот отображается на фоне сцены. Поверх её рисуется "след" оставляемый алгоритмом. Каждый алгоритм имеет свой "след". Базовым для всех "следов" является класс suro.
 * Для создания "следа" используется метод void kreiSpuro( int IdAlgoritmo, QColor bazaKoloro) в который передаётся порядковый номер алгоритма(@see A) и основной цвет. На сцене может быть создан только один "след".
